...
class MWA2APIRepo(Repo):
    ...
    def _connect(self):
        ...
        if not self.authtoken:
            if "MUNKIREPO_AUTHTOKEN" in os.environ:
                ...
                self.authtoken = "Basic %s" % base64.b64encode(user_and_pass).decode(
                    "UTF-8"
                )
    ...
    def get(self, resource_identifier):
        ...
        if resource_identifier.startswith(("catalogs/", "manifests/", "pkgsinfo/")):
            headers = {"Accept": "application/xml"}
        else:
            headers = {}
        try:
    ...
    def put_from_local_file(self, resource_identifier, local_file_path):
        ...
        if resource_identifier.startswith(("pkgs/", "icons/")):
            ...
            headers = {"Content-type": "application/xml"}
            try:
                self._curl(url, headers=headers, method="PUT", filename=local_file_path)
            except CurlError as err:
                raise RepoError(err)
