...
def find_adobepatchinstaller_app(dirpath):
    ...
    for (path, dummy_dirs, dummy_files) in os.walk(dirpath):
        if path.endswith("AdobePatchInstaller.app"):
            setup_path = os.path.join(path, "Contents", "MacOS", "AdobePatchInstaller")
            if os.path.exists(setup_path):
                return setup_path
    return ""
    ...
    if os.path.isdir(dirpath):
        ...
        # if there's no .proxy.xml we should hope there's a Media_db.db
            ...
            if os.path.exists(db_path):
                ...
                cur = conn.cursor()
                cur.execute(
                    "SELECT value FROM PayloadData WHERE "
            ...
            for item in osutils.listdir(path):
                ...
                if payloadinfo:
                    ...
                    if (driverfolder and item == driverfolder) or (
                        media_signature and payloadinfo["AdobeCode"] == media_signature
                    ):
...
def get_adobe_package_info(installroot):
    ...
    if os.path.exists(installerxml):
        ...
        if installinfo:
            packagedescriptions = installinfo[0].getElementsByTagName(
                "PackageDescription"
            )
        ...
        if description:
            ...
            if os.path.exists(installerxml):
                ...
                installinfo = dom.getElementsByTagName("InstallInfo")
                if installinfo:
                    pkgname_elems = installinfo[0].getElementsByTagName("PackageName")
                    if pkgname_elems:
...
def parse_option_xml(option_xml_file):
    ...
    if installinfo:
        ...
        if "version" in list(installinfo[0].attributes.keys()):
            info["packager_version"] = installinfo[0].attributes["version"].value
        info["package_name"] = get_xml_text_element(installinfo[0], "PackageName")
        ...
        if medias_elements:
            ...
            if media_elements:
                for media in media_elements:
                    ...
                    product["prodVersion"] = get_xml_text_element(media, "prodVersion")
...
def get_cs5_media_signature(dirpath):
    ...
    if os.path.exists(setupxml) and os.path.isfile(setupxml):
        ...
        setup_elements = dom.getElementsByTagName("Setup")
        if setup_elements:
            media_signature_elements = setup_elements[0].getElementsByTagName(
                "mediaSignature"
            )
            if media_signature_elements:
                element = media_signature_elements[0]
...
def get_cs5_uninstall_xml(option_xml_file):
    ...
    if deployment_info:
        for info_item in deployment_info:
            deployment_uninstall = info_item.getElementsByTagName("DeploymentUninstall")
...
def count_payloads(dirpath):
    ...
    for (path, dummy_dirs, files) in os.walk(dirpath):
        if path.endswith("/payloads"):
            ...
            # are .zip files
            zip_file_count = len([item for item in files if item.endswith(".zip")])
...
def get_adobe_install_info(installdir):
    ...
    adobe_install_info = {}
    if installdir:
        adobe_install_info["media_signature"] = get_cs5_media_signature(installdir)
        ...
        if os.path.exists(option_xml_file):
            adobe_install_info["uninstallxml"] = get_cs5_uninstall_xml(option_xml_file)
            ...
            if hd_app_infos:
                ...
                for hd_payload in hd_app_infos:
                    for package in hd_payload["Packages"]:
                        ...
                        if (
                            "Condition" not in list(package.keys())
                            or package.get("Condition") == ""
                            or "[installLanguage]==en_US"
                            in package.get("Condition", "")
                        ):
                            installed_size += int(package.get("ExtractSize", 0) / 1024)
                            ...
                            installed_size += int(package.get("DownloadSize", 0) / 1024)
                ...
                product_app_infos = [
                    app for app in hd_app_infos if app["SAPCode"] in product_saps
                ]
                    ...
                    for pkg in app_info["Packages"]:
                        ...
                        if pkg.get("Type") == "core":
                            ...
                            pkg_prod_vers = [
                                prod["prodVersion"]
                                for prod in option_xml_info["products"]
                                if prod.get("hd_installer")
                                and prod["SAPCode"] == app_info["SAPCode"]
                            ][0]
                            uninstall_file_name = (
                                "_".join(
                                    [
    ...
    if os.path.exists(installapp):
        ...
        if cataloginfo:
            ...
            for payload in cataloginfo.get("payloads", []):
                if payload.get("display_name", "") == cataloginfo["display_name"]:
            ...
            if installs == []:
                ...
                for payload in cataloginfo.get("payloads", []):
                    if "AdobeCode" in payload:
                        if "LangPack" in payload.get(
                            "display_name"
                        ) or "Language Files" in payload.get("display_name"):
                            # skip Language Packs
                            continue
                        dbfile = payload["AdobeCode"] + ".db"
                        filepath = os.path.join(uninstalldir, dbfile)
