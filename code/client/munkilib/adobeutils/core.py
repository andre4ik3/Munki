    ...
    def get_current_log(self):
        ...
        if recent_adobe_log and os.path.exists(recent_adobe_log):
            if not os.path.exists(pdapp_log_path) or (
                os.path.getmtime(pdapp_log_path) < os.path.getmtime(recent_adobe_log)
            ):
    ...
    def info(self):
        ...
        if logfile:
            ...
            if output:
                ...
                if (
                    logfile not in self.payload_count
                    ...
                    for line in lines:
                        ...
                        try:
                            if logfile.endswith("PDApp.log"):
                                last_adobecode = match.group(1) + "-" + match.group(2)
                            else:
...
def run_adobe_install_tool(
    ...
    progress_monitor = AdobeInstallProgressMonitor(kind=kind, operation=operation)
    ...
    while proc.poll() is None:
        ...
        if payload_completed_count > old_payload_completed_count:
            ...
            if number_of_payloads:
                ...
                display.display_status_minor(
                    "Completed payload %s%s", payload_completed_count, payloadinfo
                )
...
def run_adobe_cpp_pkg_script(dmgpath, payloads=None, operation="install"):
    ...
    if not os.path.exists(preinstall_script):
        if operation == "install":
            display.display_error("No Adobe install script found on %s" % dmgpath)
        else:
            display.display_error("No Adobe uninstall script found on %s" % dmgpath)
        dmgutils.unmountdmg(mountpoints[0])
        return -1
    ...
    for dir_name in ["ASU" "ASU2", "ProvisioningTool", "uninstallinfo"]:
        if os.path.isdir(os.path.join(basepath, dir_name)):
            os.symlink(os.path.join(basepath, dir_name), os.path.join(tmpdir, dir_name))
    ...
    for dir_name in ["Patches", "Setup"]:
        ...
        if os.path.isdir(realdir):
            ...
            for item in osutils.listdir(realdir):
                os.symlink(os.path.join(realdir, item), os.path.join(tmpsubdir, item))

    os_version_tuple = osutils.getOsVersion(as_tuple=True)
    if os_version_tuple < (10, 11) and (
        not osutils.getconsoleuser() or osutils.getconsoleuser() == u"loginwindow"
    ):
...
def run_adobe_cs5_aamee_install(dmgpath, payloads=None):
    ...
    if deploymentmanager:
        ...
        os.symlink(
            os.path.join(basepath, "ProvisioningTool"),
        ...
        for dir_name in ["Patches", "Setup"]:
            ...
            if os.path.isdir(realdir):
                ...
                for item in osutils.listdir(realdir):
                    os.symlink(
                        os.path.join(realdir, item), os.path.join(tmpsubdir, item)
                    )
        ...
        cmd.extend(
            [
                deploymentmanager,
                "--optXMLPath=%s" % option_xml_file,
                "--setupBasePath=%s" % basepath,
                "--installDirPath=/",
                "--mode=install",
            ]
        )
...
def run_adobe_cs5_patch_installer(dmgpath, copylocal=False, payloads=None):
    ...
    if mountpoints:
        if copylocal:
            ...
            retcode = subprocess.call(["/bin/cp", "-r", mountpoints[0], updatedir])
            # unmount diskimage
            dmgutils.unmountdmg(mountpoints[0])
            if retcode:
                display.display_error("Error copying items from %s" % dmgpath)
                return -1
        ...
        if patchinstaller:
            ...
            install_cmd = [patchinstaller, "--mode=silent", "--skipProcessCheck=1"]
            retcode = run_adobe_install_tool(
                install_cmd,
                number_of_payloads,
                payloads=payloads,
...
def run_adobe_uber_tool(dmgpath, pkgname="", uninstalling=False, payloads=None):
    ...
    if mountpoints:
        ...
        if os.path.exists(ubertool):
            packagename = adobeinfo.get_adobe_package_info(installroot)["display_name"]
            action = "Installing"
            operation = "install"
            if uninstalling:
                action = "Uninstalling"
...
def update_acrobatpro(dmgpath):
    ...
    if not acrobatpatchapp_path:
        display.display_error("No Acrobat Patch app at %s", acrobatpatchapp_path)
        dmgutils.unmountdmg(installroot)
        return -1

    # some values needed by the patching script
    resources_dir = os.path.join(acrobatpatchapp_path, "Contents", "Resources")
    ...
    for line in app_list:
        ...
        if os.path.exists(pathname):
            ...
            candidates = [
                item for item in info.app_data() if item["path"].endswith("/" + appname)
            ]
        ...
        retcode = proc.poll()
        if retcode != 0:
            display.display_error("Error patching %s: %s", appname, retcode)
            break
        else:
...
def do_adobe_install(item):
    ...
    managedinstallbase = prefs.pref("ManagedInstallDir")
    itempath = os.path.join(managedinstallbase, "Cache", item["installer_item"])
