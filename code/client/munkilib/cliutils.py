...
def path2url(path):
    ...
    return urljoin("file:", pathname2url(os.path.abspath(os.path.expanduser(path))))
    ...
    if darwin_vers == 10:
        if default_text:
            ...
            return unicode_or_str(
                get_input(prompt), encoding=sys.stdin.encoding
            ) or unicode_or_str(default_text)
...
def configure(prompt_list):
    ...
    if FOUNDATION_SUPPORT:
        ...
        try:
            ...
            print("Could not save configuration to %s" % PREFSPATH, file=sys.stderr)
            raise ConfigurationSaveError
