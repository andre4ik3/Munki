...
def copy_item_to_repo(repo, itempath, vers, subdirectory=""):
    ...
    try:
        ...
        raise RepoCopyError(
            u"Unable to copy %s to %s: %s" % (itempath, destination_path_name, err)
        )
    else:
...
def copy_pkginfo_to_repo(repo, pkginfo, subdirectory=""):
    ...
    pkginfo_name = "%s-%s%s" % (pkginfo["name"], pkginfo["version"], pkginfo_ext)
    ...
    try:
        ...
        raise RepoCopyError(u"Unable to save pkginfo to %s: %s" % (pkginfo_path, err))
...
def find_matching_pkginfo(repo, pkginfo):
    ...
    try:
        ...
        if pkgsinfo_items:
            ...
            print(u"Could not get a list of existing items from the repo: %s" % err)
        return {}
    ...
    if "installer_item_hash" in pkginfo:
        matchingindexes = catdb["hashes"].get(pkginfo["installer_item_hash"])
        if matchingindexes:
            return catdb["items"][matchingindexes[0]]

    if "receipts" in pkginfo:
        pkgids = [
            item["packageid"] for item in pkginfo["receipts"] if "packageid" in item
        ]
        if pkgids:
            ...
            if possiblematches:
                ...
                for versionkey in versionlist:
                    ...
                    for pkgindex in testpkgindexes:
                        ...
                        testpkgids = [
                            item["packageid"]
                            for item in testpkginfo.get("receipts", [])
...
def generate_png_from_startosinstall_item(repo, dmg_path, pkginfo):
    ...
    if mountpoints:
        ...
        if icon_path:
            try:
                repo_icon_path = convert_and_install_icon(repo, pkginfo, icon_path)
...
def convert_and_install_icon(repo, pkginfo, icon_path, index=None):
    ...
    if result:
        try:
            ...
            raise RepoCopyError(
                u"Error uploading icon to %s: %s" % (repo_png_path, err)
            )
    else:
...
def copy_icon_to_repo(repo, iconpath):
    ...
    if destination_path_name in icon_list:
        ...
        try:
            ...
            raise RepoCopyError(
                u"Could not remove existing %s: %s" % (destination_path_name, err)
            )
    ...
    try:
        ...
        raise RepoCopyError(
            u"Unable to copy %s to %s: %s" % (iconpath, destination_path_name, err)
        )
...
def extract_and_copy_icon(repo, installer_item, pkginfo, import_multiple=True):
    ...
    if installer_type == "copy_from_dmg":
        ...
        return generate_png_from_startosinstall_item(repo, installer_item, pkginfo)
