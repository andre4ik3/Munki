...
def verify_pkginfo(pkginfo_ref, pkginfo, pkgs_list, errors):
    ...
    if not "installer_item_location" in pkginfo:
        errors.append("WARNING: %s is missing installer_item_location" % pkginfo_ref)
        return False

    # Try to form a path and fail if the
    # installer_item_location is not a valid type
    try:
        installeritempath = os.path.join("pkgs", pkginfo["installer_item_location"])
    except TypeError:
        errors.append("WARNING: invalid installer_item_location in %s" % pkginfo_ref)
        return False
        ...
        for repo_pkg in pkgs_list:
            if installeritempath.lower() == repo_pkg.lower():
                errors.append(
                    ...
                    % (pkginfo_ref, pkginfo["installer_item_location"], repo_pkg)
                )
    ...
    if uninstaller_type in ["AdobeCCPUninstaller"]:
        ...
        if not "uninstaller_item_location" in pkginfo:
            errors.append(
                "WARNING: %s is missing uninstaller_item_location" % pkginfo_ref
            )
            return False
    ...
    if "uninstaller_item_location" in pkginfo:
        try:
            ...
            errors.append(
                "WARNING: invalid uninstaller_item_location " "in %s" % pkginfo_ref
            )
            return False
            ...
            for repo_pkg in pkgs_list:
                if uninstalleritempath.lower() == repo_pkg.lower():
                    errors.append(
                        ...
                        "depending on the case-sensitivity of the "
                        "underlying filesystem."
                        % (pkginfo_ref, pkginfo["uninstaller_item_location"], repo_pkg)
                    )
                    found_caseinsensitive_match = True
                    break
            if not found_caseinsensitive_match:
...
def process_pkgsinfo(repo, options, output_fn=None):
    ...
    try:
        ...
        raise MakeCatalogsError(u"Error getting list of pkgsinfo items: %s" % err)
    ...
    try:
        ...
        raise MakeCatalogsError(u"Error getting list of pkgs items: %s" % err)
    ...
    for pkginfo_ref in pkgsinfo_list:
        ...
        for catalogname in pkginfo.get("catalogs", []):
            if not catalogname:
                errors.append("WARNING: %s has an empty catalogs array!" % pkginfo_ref)
                continue
    ...
    if duplicate_catalogs:
        errors.append(
            "WARNING: There are catalogs with names that differ only "
            "by case. This may cause issues depending on the case-"
            "sensitivity of the underlying filesystem: %s" % duplicate_catalogs
        )
...
def makecatalogs(repo, options, output_fn=None):
    ...
    catalogs, catalog_errors = process_pkgsinfo(repo, options, output_fn=output_fn)
    ...
    for key in catalogs:
        ...
        if catalogs[key] != "":
            ...
            try:
                ...
                errors.append(u"Failed to create catalog %s: %s" % (key, err))
        else:
            errors.append(
    ...
    if icons:
        ...
        try:
            ...
            errors.append(u"Failed to create %s: %s" % (icon_hashes_plist, err))
