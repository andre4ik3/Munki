...
def get_catalog_info_from_path(pkgpath, options):
    ...
    if cataloginfo:
        ...
        cataloginfo = adobeinfo.getAdobeCatalogInfo(pkgpath, options.pkgname or "")
    return cataloginfo
...
def get_catalog_info_from_dmg(dmgpath, options):
    ...
    * Unmounts the disk image if it wasn't already mounted
    ...
    if not cataloginfo:
        ...
        if (
            install_macos_app
            and options.print_warnings
            and osinstaller.install_macos_app_is_stub(install_macos_app)
        ):
            print(
    ...
    if not cataloginfo:
        ...
        if options.item:
            ...
            if os.path.exists(itempath) and itempath.startswith(mountpoints[0]):
                ...
                raise PkgInfoGenerationError("%s not found on disk image." % item)
        else:
        ...
        if iteminfo:
            ...
            if options.destinationpath:
                ...
                iteminfo["path"] = os.path.join("/Applications", dest_item_filename)
            cataloginfo = {}
            ...
            cataloginfo["version"] = iteminfo.get(version_comparison_key, "0")
...
def getiteminfo(itempath):
    ...
    if pkgutils.isApplication(itempath):
        ...
        if "LSMinimumSystemVersion" in plist:
            ...
            versions = [
                item[1]
                for item in plist["LSMinimumSystemVersionByArchitecture"].items()
            ]
            highest_version = str(
                max([pkgutils.MunkiLooseVersion(version) for version in versions])
            )
            infodict["minosversion"] = highest_version
        elif "SystemVersionCheck:MinimumSystemVersion" in plist:
            infodict["minosversion"] = plist["SystemVersionCheck:MinimumSystemVersion"]

    elif os.path.exists(
        os.path.join(itempath, "Contents", "Info.plist")
    ) or os.path.exists(os.path.join(itempath, "Resources", "Info.plist")):
    ...
    if (
        not infodict.get("CFBundleShortVersionString")
        or infodict["CFBundleShortVersionString"][0] not in "0123456789"
    ):
    ...
    if (
        "CFBundleShortVersionString" not in infodict
        and "CFBundleVersion" not in infodict
    ):
        infodict["type"] = "file"
        infodict["path"] = itempath
...
def makepkginfo(installeritem, options):
    ...
    if installeritem:
        if not os.path.exists(installeritem):
            raise PkgInfoGenerationError("File %s does not exist" % installeritem)
        ...
        if pkgutils.hasValidDiskImageExt(installeritem):
            ...
            if pkginfo and pkginfo.get("installer_type") == "AdobeCS5Installer":
            ...
            if not pkginfo:
                raise PkgInfoGenerationError(
                    "Could not find a supported installer item in %s!" % installeritem
                )
            ...
            if not pkginfo:
                raise PkgInfoGenerationError(
                    "%s doesn't appear to be a valid installer item!" % installeritem
                )
        ...
        if (
            pkginfo.get("installer_type") == "AdobeCCPInstaller"
            and not options.uninstalleritem
        ) and options.print_warnings:
            print(
        ...
        for fitem in options.file:
            ...
            if os.path.exists(fitem):
                ...
                if "CFBundleShortVersionString" in iteminfodict:
                    thisitemversion = iteminfodict["CFBundleShortVersionString"]
                ...
                print("Item %s doesn't exist. Skipping." % fitem, file=sys.stderr)
    ...
    if "installs" in pkginfo:
        ...
        item_minosversions.append(pkgutils.MunkiLooseVersion(default_minosversion))
        if "minimum_os_version" in pkginfo:
            # handle case where value may have been set (e.g. flat package)
            item_minosversions.append(
                pkgutils.MunkiLooseVersion(pkginfo["minimum_os_version"])
            )
    ...
    if options.installer_environment:
        try:
            installer_environment_dict = dict(
                (k, v)
                for k, v in (kv.split("=") for kv in options.installer_environment)
            )
        except Exception:
...
def check_mode(option, opt, value, parser):
    ...
    for mode in modes:
        if rex.match(mode):
            ...
            raise optparse.OptionValueError("option %s: invalid mode: %s" % (opt, mode))
...
def add_option_groups(parser):
    ...
    default_override_options.add_option(
        "--name", metavar="NAME", help="Name of the package."
    )
    default_override_options.add_option(
        "--displayname", metavar="DISPLAY_NAME", help="Display name of the package."
    )
    default_override_options.add_option(
        "--description",
        metavar="STRING|PATH",
    ...
    default_override_options.add_option(
        "--pkgvers", metavar="PACKAGE_VERSION", help="Version of the package."
    )
    default_override_options.add_option(
        "--RestartAction",
        metavar="ACTION",
    ...
    default_override_options.add_option(
        ...
        metavar="METHOD|PATH",
        help=(
            "Specify an 'uninstall_method' for the package. "
    ...
    script_options = optparse.OptionGroup(
        parser, "Script Options", "All scripts are read and embedded into the pkginfo."
    )
    script_options.add_option(
        ...
        metavar="SCRIPT_PATH",
        help=(
            "Path to an optional installcheck script to be "
    ...
    script_options.add_option(
        ...
        metavar="SCRIPT_PATH",
        help=(
            "Path to an optional uninstallcheck script to be "
    ...
    script_options.add_option(
        ...
        metavar="SCRIPT_PATH",
        help=(
            "Path to an optional preinstall script to be "
    ...
    script_options.add_option(
        ...
        metavar="SCRIPT_PATH",
        help=(
            "Path to an optional postinstall script to be "
    ...
    script_options.add_option(
        ...
        metavar="SCRIPT_PATH",
        help=(
            "Path to an optional preuninstall script to be run "
    ...
    script_options.add_option(
        ...
        metavar="SCRIPT_PATH",
        help=(
            "Path to an optional postuninstall script to be run "
    ...
    script_options.add_option(
        ...
        metavar="SCRIPT_PATH",
        help=(
            "Path to an uninstall script to be run in order " "to uninstall this item."
        ),
    )
    ...
    dragdrop_options.add_option(
        ...
        dest="item",
        help=(
            "Name or relative path of the item to be installed. "
    ...
    dragdrop_options.add_option(
        ...
        metavar="PATH",
        help=(
            "Path to which the item should be copied. Defaults to " '"/Applications".'
        ),
    )
    dragdrop_options.add_option(
        "--destinationitemname",
        ...
        dest="destitemname",
        help=(
            "Alternate name for which the item should be copied as. "
    ...
    dragdrop_options.add_option(
        ...
        dest="user",
        help=(
            "Sets the owner of the copied item. "
    ...
    dragdrop_options.add_option(
        ...
        dest="group",
        help=(
            "Sets the group of the copied item. "
    ...
    dragdrop_options.add_option(
        ...
        callback=check_mode,
        help=(
            "Sets the mode of the copied item. "
    ...
    apple_options.add_option(
        ...
        "-p",
        help=(
            "If the installer item is a disk image containing multiple "
    ...
    apple_options.add_option(
        ...
        action="store_true",
        help=(
            "Generate installer choices for metapackages. "
    ...
    apple_options.add_option(
        ...
        metavar="KEY=VALUE",
        help=(
            "Specifies key/value pairs to set environment variables for use "
    ...
    adobe_options.add_option(
        ...
        "--uninstallpkg",
        "-U",
        metavar="UNINSTALLERITEM",
        dest="uninstalleritem",
        help=(
    ...
    forced_unattended_options.add_option(
        ...
        metavar="DATE",
        help=(
            "Specify a date, in local time, after which the package will "
    ...
    gen_installs_options = optparse.OptionGroup(parser, "Generating 'installs' items")
    gen_installs_options.add_option(
        ...
        metavar="PATH",
        help=(
            "Path to a filesystem item installed by this package, typically "
    ...
    apple_update_metadata_options.add_option(
        ...
        metavar="PRODUCTKEY",
        help=(
            "Specify an Apple update 'productKey' used to manipulate "
    ...
    additional_options.add_option(
        ...
        action="store_true",
        help=(
            "Indicates this package should be automatically removed if it is "
    ...
    additional_options.add_option(
        ...
        action="store_true",
        help=(
            "Indicates this package should be an OnDemand package "
            "not listed in any applicable 'managed_installs'."
        ),
    )
    additional_options.add_option(
        ...
        metavar="VERSION",
        help=(
            "Minimum version of munki required to perform installation. "
    ...
    additional_options.add_option(
        ...
        metavar="PKG_NAME",
        help=(
            "Specifies a package for which the current package is an update. "
    ...
    additional_options.add_option(
        ...
        metavar="PKG_NAME",
        help=(
            "Specifies a package required by the current package. Can be "
    ...
    additional_options.add_option(
        ...
        metavar="APP_NAME",
        help=(
            "Specifies an application that blocks installation. Can be "
    ...
    additional_options.add_option(
        ...
        help=(
            "Specifies in which catalog the item should appear. The default "
    ...
    additional_options.add_option(
        ...
        metavar="STRING|PATH",
        help=(
            "Specifies administrator provided notes to be embedded into the "
    ...
    additional_options.add_option(
        ...
        action="store_true",
        help=(
            "Indicates this pkginfo should have an 'installer_type' of "
