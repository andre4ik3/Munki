...
def convertIconToPNG(
    icon_path, destination_path, desired_pixel_height=350, desired_dpi=72
):
    ...
    for index in range(number_of_images):
        try:
            properties = CGImageSourceCopyPropertiesAtIndex(image_source, index, None)
            ...
            if (
                not candidate
                or (height < desired_pixel_height and height > candidate["height"])
                or (height >= desired_pixel_height and height < candidate["height"])
...
def findIconForApp(app_path):
    ...
    if not os.path.exists(app_path):
        return None
    try:
        info = readPlist(os.path.join(app_path, u"Contents/Info.plist"))
    ...
    if result == 0:
        for pkg in pkg_dict:
            ...
            if err == 0:
                ...
                display.display_error(u"pax could not read files from %s", archive_path)
                return []
    else:
