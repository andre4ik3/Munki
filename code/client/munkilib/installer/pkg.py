...
def pkg_needs_restart(pkgpath, options):
    ...
    return restartaction == "RequireRestart" or restartaction == "RecommendRestart"
...
def _run_installer(cmd, env_vars, packagename):
    ...
    try:
        ...
        display.display_error("Error with launchd job (%s): %s", cmd, str(err))
    ...
    for item in installitems:
        ...
        if pkgutils.hasValidDiskImageExt(item):
            ...
            for mountpoint in mountpoints:
                ...
                (retcode, needsrestart) = installall(mountpoint, options=options)
                if needsrestart:
                    restartflag = True
                if retcode:
        ...
        if pkgutils.hasValidInstallerItemExt(item):
            (retcode, needsrestart) = install(itempath, options=options)
            if needsrestart:
                restartflag = True
            if retcode:
