...
def insert_bomvalues_into_pkgdb(bom_line, pkgkey, ppath, curs):
    ...
    try:
        if path != ".":
            ...
            values_t = (path,)
            row = curs.execute(
                "SELECT path_key from paths where path = ?", values_t
            ).fetchone()
            if not row:
...
def import_package(packagepath, curs):
    ...
    if not os.path.isdir(packagepath):
        ...
        if pkgname != "BSD.pkg":
            display.display_warning("%s is not a valid receipt. Skipping.", packagepath)
        return
    ...
    if not os.path.exists(bompath):
        ...
        if not os.path.exists(bompath):
            display.display_warning("%s has no BOM file. Skipping.", packagepath)
            return

    if not os.path.exists(infopath):
        display.display_warning("%s has no Info.plist. Skipping.", packagepath)
        return
    ...
    pkgid = plist.get("CFBundleIdentifier", plist.get("Bundle identifier", pkgname))
    # 'Bundle versions string, short' is a weird Casper Composer thing
    vers = plist.get(
        "CFBundleShortVersionString", plist.get("Bundle versions string, short", "1.0")
    )
...
def init_database(forcerebuild=False):
    ...
    display.display_status_minor("Gathering information on installed packages")

    if os.path.exists(PACKAGEDB):
        try:
            os.remove(PACKAGEDB)
        except (OSError, IOError):
            display.display_error("Could not remove out-of-date receipt database.")
            return False
    ...
    if os.path.exists(receiptsdir):
        receiptlist = [
            item for item in osutils.listdir(receiptsdir) if item.endswith(u".pkg")
        ]

    bomsdir = u"/Library/Receipts/boms"
    bomslist = []
    if os.path.exists(bomsdir):
        bomslist = [item for item in osutils.listdir(bomsdir) if item.endswith(".bom")]
...
def getpkgkeys(pkgnames):
    ...
    for pkg in pkgnames:
        values_t = (pkg,)
        display.display_debug1("select pkg_key from pkgs where pkgid = %s", pkg)
        pkg_keys = curs.execute(
            "select pkg_key from pkgs where pkgid = ?", values_t
        ).fetchall()
        if not pkg_keys:
            # try pkgname
            display.display_debug1("select pkg_key from pkgs where pkgname = %s", pkg)
            pkg_keys = curs.execute(
                "select pkg_key from pkgs where pkgname = ?", values_t
            ).fetchall()
        if not pkg_keys:
...
def getpathstoremove(pkgkeylist):
    ...
    display.display_status_minor("Determining which filesystem items to remove")
...
def remove_receipts(pkgkeylist, noupdateapplepkgdb):
    ...
    for pkgkey in pkgkeylist:
        ...
        pkgkey_t = (pkgkey,)
        row = curs.execute(
            "SELECT pkgname, pkgid from pkgs where pkg_key = ?", pkgkey_t
        ).fetchone()
        if row:
            pkgname = row[0]
            pkgid = row[1]
            ...
            if receiptpath and os.path.exists(receiptpath):
                ...
                dummy_retcode = subprocess.call(["/bin/rm", "-rf", receiptpath])

        # remove pkg info from our database
        display.display_detail("Removing package data from internal database...")
    ...
    # Apple DB...
    display.display_detail("Removing unused paths from internal package database...")
    curs.execute(
...
def remove_filesystem_items(removalpaths, forcedeletebundles):
    ...
    display.display_status_minor("Removing %s filesystem items" % removalcount)
            ...
            if os.path.isdir(pathtoremove) and not os.path.islink(pathtoremove):
                ...
                if diritems == []:
                    ...
                    if forcedeletebundles and is_bundle(pathtoremove):
                        ...
                        # directories.
                        if not inside_bundle(pathtoremove) or not forcedeletebundles:
                            msg = (
    ...
    if removalerrors:
        ...
        display.display_info("There were problems removing some filesystem items.")
        display.display_info("---------------------------------------------------")
...
def removepackages(
    ...
    """
    if pkgnames == []:
        display.display_error("You must specify at least one package to remove!")
        return -2
