...
def get_appleupdates():
    ...
    if os.path.exists(appleupdatesfile) and (
        prefs.pref("InstallAppleSoftwareUpdates")
        or prefs.pref("AppleSoftwareUpdatesOnly")
    ):
        try:
...
def get_pending_update_info():
    ...
    data["PendingUpdateCount"] = (
        data["install_count"] + data["removal_count"] + data["apple_update_count"]
    )
...
def save_pending_update_times():
    ...
    installinfo = get_installinfo()
    install_names = [item["name"] for item in installinfo.get("managed_installs", [])]
    removal_names = [item["name"] for item in installinfo.get("removals", [])]

    appleupdatesinfo = get_appleupdates()
    appleupdate_names = [
        item["name"] for item in appleupdatesinfo.get("AppleUpdates", [])
    ]
    update_names = {
    ...
    for category in update_names:
        ...
        for name in update_names[category]:
            if (
                category in prior_pending_updates
                and name in prior_pending_updates[category]
            ):
...
def display_update_info():
    ...
    if installcount:
        ...
        display.display_info("The following items will be installed or upgraded:")
    for item in installinfo.get("managed_installs", []):
        if item.get("installer_item"):
            display.display_info(
                "    + %s-%s", item.get("name", ""), item.get("version_to_install", "")
            )
    ...
    if installcount == 0 and removalcount == 0:
        display.display_info("No changes to managed software are available.")
...
def force_install_package_check():
    ...
    if prefs.pref("InstallAppleSoftwareUpdates") or prefs.pref(
    ...
    for installinfo_plist in installinfo_types:
        ...
        for i in xrange(len(installinfo.get(pl_dict, []))):
            ...
            force_install_after_date = info.subtract_tzoffset_from_date(
                force_install_after_date
            )
            display.display_debug1(
                "Forced install for %s at %s", install["name"], force_install_after_date
            )
            if now >= force_install_after_date:
                ...
                if install.get("RestartAction"):
                    if install["RestartAction"] == "RequireLogout":
                        ...
                        install["RestartAction"] == "RequireRestart"
