...
def pref(pref_name):
    ...
    """
    return CFPreferencesCopyAppValue(pref_name, APPLE_SOFTWARE_UPDATE_PREFS_DOMAIN)
...
def set_custom_catalogurl(catalog_url):
    ...
    if os_version_tuple < (10, 11):
        ...
        if not CFPreferencesSynchronize(
            ...
            display.display_error("Error setting com.apple.SoftwareUpdate CatalogURL.")
    else:
...
def reset_original_catalogurl():
    ...
    if os_version_tuple < (10, 11):
        ...
        if original_catalog_url:
            # use softwareupdate --set-catalog
            cmd = ["/usr/sbin/softwareupdate", "--set-catalog", original_catalog_url]
        else:
    ...
    if not CFPreferencesSynchronize(
        ...
        display.display_error("Error resetting com.apple.SoftwareUpdate CatalogURL.")
