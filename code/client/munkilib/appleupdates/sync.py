...
class AppleUpdateSync(object):
    ...
    def __init__(self):
        ...
        if os.path.exists(real_cache_dir):
            if not os.path.isdir(real_cache_dir):
                display.display_error("%s exists but is not a dir.", real_cache_dir)
        else:
        ...
        try:
            ...
            if os.path.exists(self.cache_dir):
                ...
                new_name = os.path.join(
                    "/tmp",
                    ("munki_swupd_cache_moved_%s" % time.strftime("%Y.%m.%d.%H.%M.%S")),
                )
    ...
    def cache_apple_catalog(self):
        ...
        if os.path.exists(self.apple_download_catalog_path):
            stored_os_vers = str(
                fetch.getxattr(self.apple_download_catalog_path, XATTR_OS_VERS)
            )
        ...
        try:
            ...
            xattr.setxattr(
                self.apple_download_catalog_path, XATTR_OS_VERS, os_vers.encode("UTF-8")
            )
    ...
    def rewrite_product_urls(self, product, rewrite_pkg_urls=False):
        ...
        for package in product.get("Packages", []):
            ...
            if "MetadataURL" in package:
                package["MetadataURL"] = self.rewrite_url(package["MetadataURL"])
        ...
        for dist_lang in list(distributions.keys()):
            distributions[dist_lang] = self.rewrite_url(distributions[dist_lang])
    ...
    def retrieve_url_to_cache_dir(self, full_url, copy_only_if_missing=False):
        ...
        if not os.path.exists(local_dir_path):
            try:
                ...
                raise ReplicationError(oserr)
        try:
            self.get_su_resource(full_url, local_file_path, resume=True)
    ...
    def cache_update_metadata(self, product_ids):
        ...
        for product_key in product_ids:
            ...
            if product_key not in catalog["Products"]:
                if product_key.startswith("MSU_UPDATE_"):
                    ...
                    display.display_info(
                        "Skipping metadata caching for product ID %s" % product_key
                    )
                else:
                    display.display_warning(
                        "Could not cache metadata for product ID %s" % product_key
                    )
                continue
            product = catalog["Products"][product_key]
            if "ServerMetadataURL" in product:
            ...
            for dist_lang in distributions.keys():
                if processes.stop_requested():
                    return
                display.display_status_minor(
                    "Caching %s distribution for product ID %s", dist_lang, product_key
                )
                dist_url = distributions[dist_lang]
                try:
                    self.retrieve_url_to_cache_dir(dist_url, copy_only_if_missing=True)
        ...
        FoundationPlist.writePlist(catalog, self.local_download_catalog_path)
    ...
    def _preferred_localization(self, list_of_localizations):
        ...
        # pylint: disable=no-self-use
        localization_preferences = prefs.pref("AppleSoftwareUpdateLanguages") or [
            "English"
        ]
        preferred_langs = NSBundle.preferredLocalizationsFromArray_forPreferences_(
    ...
    def distribution_for_product_key(self, product_key, language=None):
        ...
        if product:
            ...
            if distributions:
                ...
                dist_path = os.path.join("/Library/Updates", product_key, filename)
                ...
                try:
                    ...
                    display.display_error("Could not retrieve %s: %s", url, err)
        return None
