...
def get_restart_action(restart_action_list):
    ...
    restart_actions = ["None", "RequireLogout", "RecommendRestart", "RequireRestart"]
    ...
    if localizations:
        ...
        if string_elements:
            ...
            if strings.firstChild:
                ...
                except (AttributeError, FoundationPlist.FoundationPlistException):
                    strings_data = {}
    return strings_data
...
def populate_pkgs_from_pkg_refs(dom, pkgs):
    ...
    for pkg_ref in dom_pkg_refs:
        ...
        if "onConclusion" in list(pkg_ref.attributes.keys()):
            pkgs[pkg_id]["RestartAction"] = pkg_ref.attributes["onConclusion"].value
        if "version" in list(pkg_ref.attributes.keys()):
            pkgs[pkg_id]["version"] = pkg_ref.attributes["version"].value
        ...
        if "packageIdentifier" in list(pkg_ref.attributes.keys()):
            pkgs[pkg_id]["packageid"] = pkg_ref.attributes["packageIdentifier"].value
...
def get_su_choice_and_pkgs(dom):
    ...
    for outline in choice_outlines:
        if (
            "ui" in list(outline.attributes.keys())
            and outline.attributes["ui"].value == "SoftwareUpdate"
        ):
...
def parse_su_dist(filename):
    ...
    try:
        ...
        display.display_error("Invalid XML in %s", filename)
        return None
    except IOError as err:
        display.display_error("Error reading %s: %s", filename, err)
        return None
    ...
    restart_actions = [
        pkg["RestartAction"] for pkg in pkgs.values() if "RestartAction" in pkg
    ]
