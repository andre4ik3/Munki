...
def set_bootstrap_mode():
    ...
    try:
        ...
        reset_fde_autologin()
        raise SetupError("Could not create bootstrapping flag file: %s" % err)
...
def clear_bootstrap_mode():
    ...
    if os.path.exists(constants.CHECKANDINSTALLATSTARTUPFLAG):
        try:
            ...
            raise SetupError("Could not remove bootstrapping flag file: %s" % err)
