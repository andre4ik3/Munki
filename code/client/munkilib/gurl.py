...
class Gurl(NSObject):
    ...
    def start(self):
        ...
        if os.path.isfile(self.destination_path):
            ...
            if (
                self.can_resume
                and "expected-length" in stored_data
    ...
    def storeHeaders_(self, headers):
        ...
        try:
            ...
            self.log(
                "Could not store metadata to %s: %s" % (self.destination_path, err)
            )
        ...
        if not self.destination and self.destination_path:
            if self.status == 206 and self.resume:
                ...
                if (
                    ...
                    self.log("Restarting download of %s" % self.destination_path)
                    os.unlink(self.destination_path)
                    self.start()
                    return
                # try to resume
    ...
    def handleChallenge_withCompletionHandler_(self, challenge, completionHandler):
        ...
        if (
            ...
            self.log("Will attempt to authenticate")
            credential = (
                NSURLCredential.alloc().initWithIdentity_certificates_persistence_(
