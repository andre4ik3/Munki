...
def import_middleware(
    ...
    for filename in os.listdir(munki_dir):
        if filename.startswith(file_prefix) and os.path.splitext(filename)[1] == ".py":
...
postware = None
import_middleware(
    file_prefix="postware",
    globals_prefix="postware",
    required_function_name="process_post_fetch",
)
...
def get_url(
        ...
        while True:
            ...
            if (
                ...
                if connection.percentComplete != stored_percent_complete:
                    ...
                    display.display_percent_done(stored_percent_complete, 100)
                ...
                display.display_detail("Bytes received: %s", stored_bytes_received)
            if connection_done:
                break
    ...
    if connection.error is not None:
        ...
        if connection.SSLerror:
            display.display_detail("SSL error detail: %s", str(connection.SSLerror))
...
def getResourceIfChangedAtomically(
    ...
    if resume and expected_hash and os.path.isfile(destinationpath):
        ...
        if xattr_hash == expected_hash:
            ...
            return False
        elif prefs.pref("PackageVerificationMode").lower() in ["hash_strict", "hash"]:
            try:
                os.unlink(destinationpath)
            except OSError:
                pass
        munkilog.log(
            "Cached item does not match hash in catalog, "
    ...
    if url_parse.scheme in ["http", "https"]:
        ...
        raise Error("Unsupported scheme for %s: %s" % (url, url_parse.scheme))

    if changed and verify:
        (verify_ok, fhash) = verifySoftwarePackageIntegrity(
            destinationpath, expected_hash, always_hash=True
        )
        if not verify_ok:
            try:
                os.unlink(destinationpath)
...
def getFileIfChangedAtomically(path, destinationpath):
    ...
    if st_dst is not None and (
        st_src.st_mtime == st_dst.st_mtime and st_src.st_size == st_dst.st_size
    ):
        return False
    ...
    try:
        ...
        if err.args[0] == errno.ENOENT:
            ...
            raise FileCopyError("Removing %s: %s" % (tmp_destinationpath, str(err)))
...
def getHTTPfileIfChangedAtomically(
    url,
    destinationpath,
    custom_headers=None,
    message=None,
    ...
    if header["http_result_code"] == "304":
        ...
        display.display_debug1("%s already exists and is up-to-date.", destinationpath)
    ...
    if not mode:
        ...
        if item_hash:
            ...
            if mode.lower() == "hash_strict":
                display.display_error(
                    "Reference hash value for %s is missing in catalog." % item_name
                )
                return (False, chash)
