...
KEYCHAIN_DIRECTORY = os.path.join(prefs.pref("ManagedInstallDir"), "Keychains")
...
def read_file(pathname):
    ...
    try:
        ...
        display.display_error("Could not read %s: %s", pathname, err)
        return ""
...
def write_file(stringdata, pathname):
    ...
    try:
        ...
        display.display_error("Couldn't write %s to %s: %s", stringdata, pathname, err)
        return ""
...
def pem_cert_bytes(cert_path):
    ...
    if not (
        ("-----BEGIN CERTIFICATE-----" in cert_data)
        and ("-----END CERTIFICATE-----" in cert_data)
    ):
        raise Exception("Certificate does not appear to be .pem file")
    core_data = (
        cert_data.split("-----BEGIN CERTIFICATE", 1)[-1]
        .replace("\r", "\n")
        .split("-----\n", 1)[-1]
        .split("\n-----END CERTIFICATE-----", 1)[0]
    )
...
def get_munki_server_cert_info():
    ...
    if prefs.pref("SoftwareRepoCACertificate"):
        cert_info["ca_cert_path"] = prefs.pref("SoftwareRepoCACertificate")
    if cert_info["ca_cert_path"] is None:
        ca_cert_path = os.path.join(prefs.pref("ManagedInstallDir"), "certs", "ca.pem")
...
def get_munki_client_cert_info():
    ...
    if prefs.pref("UseClientCertificate"):
        cert_info["client_cert_path"] = prefs.pref("ClientCertificatePath") or None
...
def add_ca_certs_to_system_keychain(cert_info=None):
    ...
    for cert in certs_to_add:
        ...
        try:
            output = security("add-trusted-cert", "-d", "-k", system_keychain, cert)
            if output:
...
def make_client_keychain(cert_info=None):
    ...
    if os.geteuid() != 0:
        display.display_error("Can't make our client keychain unless we are root!")
        return
    ...
    keychain_pass = prefs.pref("KeychainPassword") or DEFAULT_KEYCHAIN_PASSWORD
    ...
    try:
        output = security("create-keychain", "-p", keychain_pass, abs_keychain_path)
        if output:
            display.display_debug2(output)
    except SecurityError as err:
        display.display_error(
    ...
    if client_key_path:
        ...
        if write_file(cert_data + key_data, combined_pem):
            ...
            display.display_error("Could not combine client cert and key for import!")
    else:
    ...
    if client_cert_file:
        ...
        try:
            output = security("import", client_cert_file, "-A", "-k", abs_keychain_path)
            if output:
                display.display_debug2(output)
        except SecurityError as err:
            display.display_error("Could not import %s: %s", client_cert_file, err)
    if combined_pem:
    ...
    if not id_hash:
        display.display_error("Cannot create keychain identity preference.")
    else:
        ...
        try:
            output = security("default-keychain", "-s", abs_keychain_path)
            if output:
                display.display_debug2(
        ...
        for url in site_urls:
            try:
                display.display_debug2("Adding identity preference for %s..." % url)
                output = security(
                ...
                display.display_error(
                    "Setting identity preference for %s failed: %s" % (url, err)
                )
        if default_keychain:
            # We originally had a different default, set it back
            output = security("default-keychain", "-s", default_keychain)
            if output:
                display.display_debug2(
                    "Attempting to set default keychain to %s resulted in: %s",
                    default_keychain,
...
def add_to_keychain_list(keychain_path):
    ...
    if not keychain_path in search_keychains:
        ...
        try:
            output = security("list-keychains", "-d", "user", "-s", *search_keychains)
            if output:
                display.display_debug2(output)
            added_keychain = True
        except SecurityError as err:
            display.display_error(
                "Could not add keychain %s to keychain list: %s", keychain_path, err
            )
            added_keychain = False
...
def remove_from_keychain_list(keychain_path):
    ...
    if keychain_path in search_keychains:
        ...
        display.display_debug1("Removing %s from search path...", keychain_path)
        filtered_keychains = [
            keychain for keychain in search_keychains if keychain != keychain_path
        ]
        try:
            output = security("list-keychains", "-d", "user", "-s", *filtered_keychains)
            if output:
                display.display_debug2(output)
        except SecurityError as err:
            display.display_error("Could not set new keychain list: %s", err)
...
def unlock_and_set_nonlocking(keychain_path):
    ...
    keychain_pass = prefs.pref("KeychainPassword") or DEFAULT_KEYCHAIN_PASSWORD
    try:
        output = security("unlock-keychain", "-p", keychain_pass, keychain_path)
        if output:
            display.display_debug2(output)
    except SecurityError as err:
        # some problem unlocking the keychain.
        display.display_error("Could not unlock %s: %s.", keychain_path, err)
        # delete it
        try:
            os.unlink(keychain_path)
        except OSError as err:
            display.display_error("Could not remove %s: %s.", keychain_path, err)
        return
    try:
        ...
        display.display_error(
            "Could not set keychain settings for %s: %s", keychain_path, err
        )
...
def debug_output():
    ...
    try:
        ...
        if os.path.exists(keychainfile):
            ...
            display.display_debug1(security("show-keychain-info", keychainfile))
    except SecurityError as err:
...
def security(verb_name, *args):
    ...
    proc = subprocess.Popen(
        cmd, shell=False, bufsize=-1, stdout=subprocess.PIPE, stderr=subprocess.PIPE
    )
...
def get_keychain_path():
    ...
    keychain_name = prefs.pref("KeychainName") or DEFAULT_KEYCHAIN_NAME
    ...
    if os.path.dirname(keychain_name):
        ...
        keychain_name = os.path.basename(keychain_name).strip() or DEFAULT_KEYCHAIN_NAME
    # Correct the filename to include '.keychain' if not already present
    if not keychain_name.lower().endswith(".keychain"):
        keychain_name += ".keychain"
    keychain_path = os.path.realpath(os.path.join(KEYCHAIN_DIRECTORY, keychain_name))
    ...
    def __init__(self):
        ...
        if client_certs_exist() and os.path.exists(self.keychain_path):
            ...
            try:
                ...
                display.display_error(
                    "Could not remove pre-existing %s: %s" % (self.keychain_path, err)
                )
