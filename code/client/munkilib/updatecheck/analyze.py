    ...
    for listitem in thelist:
        try:
            if listitem["name"] == item_pl["name"]:
                ...
                if listitem.get("installed") and (
                    compare.compare_versions(listitem.get("installed_version"), vers)
                    in (1, 2)
                ):
                    return True
                if compare.compare_versions(
                    listitem.get("version_to_install"), vers
                ) in (1, 2):
                    return True
        except KeyError:
    ...
    for section in sections:
        if itemname in installinfo[section]:
            display.display_debug1(
                "%s has already been processed for %s.", itemname, description[section]
            )
            return True
    return False
...
def process_managed_update(manifestitem, cataloglist, installinfo):
    ...
    display.display_debug1("* Processing manifest item %s for update", manifestitemname)
...
def process_optional_install(manifestitem, cataloglist, installinfo):
    ...
    if already_processed(
        manifestitemname,
        installinfo,
        ["optional_installs", "processed_installs", "processed_uninstalls"],
    ):
        return
    ...
    for item in installinfo["optional_installs"]:
        if manifestitemname == item["name"]:
            display.display_debug1(
                "%s has already been processed for optional install.", manifestitemname
            )
            return

    item_pl = catalogs.get_item_detail(
        manifestitem, cataloglist, suppress_warnings=True
    )
    if not item_pl and prefs.pref("ShowOptionalInstallsForHigherOSVersions"):
        ...
        item_pl = catalogs.get_item_detail(
            manifestitem, cataloglist, skip_min_os_check=True, suppress_warnings=True
        )
        if item_pl:
            ...
            item_pl["note"] = (
                "Requires macOS version %s." % item_pl["minimum_os_version"]
            )
    ...
    if is_currently_installed:
        ...
        if not needs_update and prefs.pref("ShowOptionalInstallsForHigherOSVersions"):
    ...
    iteminfo["uninstallable"] = item_pl.get("uninstallable", False) and (
        item_pl.get("uninstall_method", "") != ""
    )
    ...
    if (
        item_pl.get("installer_item_location")
        and item_pl.get("precache")
        and not "note" in item_pl
        and (
            not "licensed_seats_available" in item_pl
            or item_pl["licensed_seats_available"]
        )
    ):
    ...
    display.display_debug1("Adding %s to the optional install list", iteminfo["name"])
...
def process_install(
    ...
    if manifestitemname in installinfo["processed_installs"]:
        ...
        return True
    elif manifestitemname_withoutversion in installinfo["processed_uninstalls"]:
        display.display_warning(
            "Will not process %s for install because it has already "
            "been processed for uninstall!",
            manifestitemname,
        )
        return False
    ...
    if item_in_installinfo(
        item_pl, installinfo["managed_installs"], vers=item_pl.get("version")
    ):
        # has this item already been added to the list of things to install?
        display.display_debug1("%s is or will be installed.", manifestitemname)
        return True
    ...
    if "requires" in item_pl:
        ...
        for item in dependencies:
            ...
            success = process_install(
                item, cataloglist, installinfo, is_managed_update=is_managed_update
            )
            if not success:
    ...
    if installed_state == 0:
        ...
        try:
            ...
            if item_pl.get("installer_type", 0) == "nopkg":
                ...
                filename = download.get_url_basename(item_pl["installer_item_location"])
            ...
            # restart or logout...
            if item_pl.get("unattended_install") or item_pl.get("forced_install"):
                if item_pl.get("RestartAction", "None") != "None":
                    display.display_warning(
                        "Ignoring unattended_install key for %s because "
                        "RestartAction is %s.",
                        item_pl["name"],
                        item_pl.get("RestartAction"),
                    )
                else:
            ...
            if is_optional_install and not installationstate.some_version_installed(
                item_pl
            ):
            ...
            display.display_warning(
                "Can't install %s because the integrity check failed.", manifestitem
            )
            iteminfo["installed"] = False
            ...
            display.display_warning("Download of %s failed: %s", manifestitem, errmsg)
            iteminfo["installed"] = False
        ...
        display.display_detail(
            "%s version %s (or newer) is already installed.", name, item_pl["version"]
        )
        update_list = []
        if not includedversion:
            ...
            compare.compare_versions(includedversion, iteminfo["installed_version"])
            == 1
        ):
...
def process_manifest_for_key(manifest, manifest_key, installinfo, parentcatalogs=None):
    ...
    if is_a_string(manifest):
        display.display_debug1(
            "** Processing manifest %s for %s", os.path.basename(manifest), manifest_key
        )
    ...
    for item in manifestdata.get("included_manifests", []):
        if item:  # only process if item is not empty
            ...
            process_manifest_for_key(
                nestedmanifestpath, manifest_key, installinfo, cataloglist
            )

    conditionalitems = manifestdata.get("conditional_items", [])
    if conditionalitems:
        display.display_debug1("** Processing conditional_items in %s", manifest)
    ...
    for item in conditionalitems:
        try:
            ...
            display.display_warning("Missing predicate for conditional_item %s", item)
            continue
        except BaseException:
            display.display_warning("Conditional item is malformed: %s", item)
            continue
...
def process_removal(manifestitem, cataloglist, installinfo):
    ...
    display.display_debug1(
        "* Processing manifest item %s for removal" % manifestitemname_withversion
    )
    ...
    # have we processed this already?
    if manifestitemname in [
        catalogs.split_name_and_version(item)[0]
    ...
    if includedversion:
        ...
        infoitems = catalogs.get_all_items_with_name(manifestitemname, cataloglist)
    ...
    for item in infoitems:
        display.display_debug2(
            "Considering item %s-%s for removal info", item["name"], item["version"]
        )
    ...
    if not uninstall_item:
        ...
        # to what's on disk
        display.display_warning(
            "Could not find uninstall info for %s.", manifestitemname_withversion
        )
        return False
    ...
    uninstall_name_w_version = "%s-%s" % (
        uninstall_item.get("name"),
        uninstall_item.get("version"),
    )
    alt_uninstall_name_w_version = "%s--%s" % (
        uninstall_item.get("name"),
        uninstall_item.get("version"),
    )
    processednames = []
    for catalogname in cataloglist:
        ...
        for item_pl in catalogs.catalogs()[catalogname]["items"]:
            ...
            if name not in processednames:
                if "requires" in item_pl:
                    if (
                        uninstall_item_name in item_pl["requires"]
                        or uninstall_name_w_version in item_pl["requires"]
                        or alt_uninstall_name_w_version in item_pl["requires"]
                    ):
                        display.display_debug1(
                            "%s requires %s, checking to see if it's " "installed...",
                            item_pl.get("name"),
                            manifestitemname,
                        )
                        if installationstate.evidence_this_is_installed(item_pl):
                            display.display_detail(
    ...
    if not dependentitemsremoved:
        display.display_warning(
            "Will not attempt to remove %s because could "
    ...
    # or logout...
    if uninstall_item.get("unattended_uninstall") or uninstall_item.get(
        "forced_uninstall"
    ):
    ...
    if packages_to_remove:
        ...
        for pkg in packages_to_remove:
            ...
            if pkg in pkgdata["pkg_references"]:
                display.display_debug1(
                    "%s references are: %s", pkg, pkgdata["pkg_references"][pkg]
                )
                if iteminfo["name"] in pkgdata["pkg_references"][pkg]:
                    ...
                    if not pkgdata["pkg_references"][pkg]:
                        ...
                        display.display_debug1("Adding %s to removal list.", pkg)
        ...
        if packages_to_really_remove:
            ...
            display.display_warning(
                "could not find unique packages to remove " "for %s", iteminfo["name"]
            )
            return False

    iteminfo["uninstall_method"] = uninstallmethod
    if uninstallmethod.startswith("Adobe"):
        if uninstallmethod == "AdobeCS5AAMEEPackage" and "adobe_install_info" in item:
            ...
            try:
                download.download_installeritem(item, installinfo, uninstalling=True)
    ...
    update_list.extend(catalogs.look_for_updates(uninstall_name_w_version, cataloglist))
    update_list.extend(
        catalogs.look_for_updates(alt_uninstall_name_w_version, cataloglist)
    )
    for update_item in update_list:
        # call us recursively...
    ...
    installinfo["removals"].append(iteminfo)
    display.display_detail(
        "Removal of %s added to ManagedInstaller tasks.", manifestitemname_withversion
    )
    return True
