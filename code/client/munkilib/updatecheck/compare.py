...
def compare_versions(thisvers, thatvers):
    ...
    """
    if pkgutils.MunkiLooseVersion(thisvers) < pkgutils.MunkiLooseVersion(thatvers):
        return VERSION_IS_LOWER
    elif pkgutils.MunkiLooseVersion(thisvers) == pkgutils.MunkiLooseVersion(thatvers):
...
def compare_application_version(app):
    ...
    if name == "" and bundleid == "":
        ...
        raise utils.Error("No path, application name or bundleid was specified!")
    ...
    appinfo = [
        ...
        if (
            item["path"]
            and (item["bundleid"] == bundleid or (name and item["name"] == name))
        )
    ]

    if not appinfo:
        # No matching apps found
        display.display_debug1("\tFound no matching applications on the startup disk.")
        return ITEM_NOT_PRESENT
    ...
    for item in appinfo:
        ...
        display.display_debug2("\tFound CFBundleIdentifier: \t %s", item["bundleid"])
...
def compare_plist_version(item):
    ...
    display.display_debug1(
        "\tChecking %s for %s %s...", filepath, version_comparison_key, versionstring
    )
    if not os.path.exists(filepath):
    ...
    if not hasattr(plist, "get"):
        display.display_debug1("plist not parsed as NSCFDictionary: %s", filepath)
        return ITEM_NOT_PRESENT
    ...
    if "version_comparison_key" in item:
        ...
        installedvers = pkgutils.getVersionString(plist, version_comparison_key)
    else:
        # default behavior
        installedvers = pkgutils.getVersionString(plist)
    if installedvers:
        display.display_debug1("\tInstalled item has version %s", installedvers)
        if minupvers:
        ...
        display.display_debug1("\tInstalled item is %s.", results[compare_result + 1])
        return compare_result
    else:
...
def get_installed_version(item_plist):
    ...
    for receipt in item_plist.get("receipts", []):
        ...
        if compare_versions(receipt.get("version", 0), item_plist["version"]) == 1:
    ...
    for install_item in install_items_with_versions:
        ...
        if (
            compare_versions(
                install_item["CFBundleShortVersionString"], item_plist["version"]
            )
            == 1
        ):
            if install_item["type"] == "application":
                ...
                display.display_debug2(
                    ...
                    name,
                    install_item.get("CFBundleIdentifier"),
                )
                try:
                    # check default location for app
                    filepath = os.path.join(
                        install_item["path"], "Contents", "Info.plist"
                    )
                    plist = FoundationPlist.readPlist(filepath)
                    return plist.get("CFBundleShortVersionString", "UNKNOWN")
                except (KeyError, FoundationPlist.NSPropertyListSerializationException):
                    ...
                    for ai_item in appinfo:
                        if (
                            "version" in ai_item
                            and compare_versions(ai_item["version"], maxversion) == 2
                        ):
                            # version is higher
