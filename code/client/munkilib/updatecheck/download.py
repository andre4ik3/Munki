...
def enough_disk_space(
    item_pl, installlist=None, uninstalling=False, warn=True, precaching=False
):
    ...
    diskspaceneeded = (
        installeritemsize - alreadydownloadedsize + installedsize + fudgefactor
    )
    ...
    if installlist:
        for item in installlist:
            ...
            if item.get("installer_item"):
                availablediskspace = availablediskspace - int(
    ...
    if warn:
        if uninstalling:
            ...
            display.display_warning(
                "There is insufficient disk space to " "download and install %s.",
                item_pl.get("name"),
            )
        display.display_warning(
            "    %sMB needed; %sMB available",
            int(diskspaceneeded / 1024),
            int(availablediskspace / 1024),
        )
    return False


def download_installeritem(item_pl, installinfo, uninstalling=False, precaching=False):
    ...
    if not location:
        raise fetch.DownloadError("No %s in item info." % download_item_key)

    # allow pkginfo preferences to override system munki preferences
    downloadbaseurl = (
        item_pl.get("PackageCompleteURL")
        or item_pl.get("PackageURL")
        or prefs.pref("PackageURL")
        or prefs.pref("SoftwareRepoURL") + "/pkgs/"
    )
...
def download_icons(item_list):
    ...
    icon_base_url = prefs.pref("IconURL") or prefs.pref("SoftwareRepoURL") + "/icons/"
        ...
        if server_icon_hash != local_hash:
            ...
            try:
                ...
                display.display_debug1(
                    "Error when retrieving icon %s from the server: %s", icon_name, err
                )
...
def download_client_resources():
    ...
    resource_base_url = (
        prefs.pref("ClientResourceURL")
        or prefs.pref("SoftwareRepoURL") + "/client_resources/"
    )
    resource_base_url = resource_base_url.rstrip("/") + "/"
    resource_dir = os.path.join(prefs.pref("ManagedInstallDir"), "client_resources")
    display.display_debug2("Client resources base URL is: %s", resource_base_url)
    # make sure local resource directory exists
    if not os.path.isdir(resource_dir):
        try:
            os.makedirs(resource_dir, 0o755)
        except OSError as err:
            display.display_error("Could not create %s" % resource_dir)
            return
    ...
    for filename in filenames:
        ...
        try:
            fetch.munki_resource(resource_url, resource_archive_path, message=message)
            downloaded_resource_path = resource_archive_path
            break
        except fetch.Error as err:
            display.display_debug1(
                "Could not retrieve client resources with name %s: %s", filename, err
            )
...
def download_catalog(catalogname):
    ...
    catalogbaseurl = (
        prefs.pref("CatalogURL") or prefs.pref("SoftwareRepoURL") + "/catalogs/"
    )
    ...
    try:
        ...
        display.display_error(
            "Could not retrieve catalog %s from server: %s", catalogname, err
        )
        return None
...
def _items_to_precache(install_info):
    ...
    precache_items = [
        ...
        and (not item.get("installed") or item.get("needs_update"))
    ]
...
def cache():
    ...
    for item in _items_to_precache(install_info):
        try:
            ...
            display.display_warning(
                u"Failed to precache the installer for %s because %s", item["name"], err
            )
...
def uncache(space_needed_in_kb):
    ...
    precached_items = [item for item in precachable_items if item in items_in_cache]
    ...
    for item in precached_items:
        ...
        try:
            ...
            display.display_warning("Could not get size of %s: %s" % (item_path, err))
            itemsize = 0
