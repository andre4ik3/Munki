...
def get_manifest(manifest_name, suppress_errors=False):
    ...
    manifestbaseurl = (
        prefs.pref("ManifestURL") or prefs.pref("SoftwareRepoURL") + "/manifests/"
    )
    ...
    manifesturl = manifestbaseurl + quote(manifest_name.encode("UTF-8"))
    ...
    try:
        dummy_value = fetch.munki_resource(manifesturl, manifestpath, message=message)
    except fetch.ConnectionError as err:
        raise ManifestServerConnectionException(err)
    except fetch.Error as err:
        if not suppress_errors:
            display.display_error(
                "Could not retrieve manifest %s from the server: %s", manifest_name, err
            )
...
def get_primary_manifest(alternate_id=""):
    ...
    elif prefs.pref("UseClientCertificate") and prefs.pref(
    ...
    if clientidentifier:
        ...
        if not manifest:
            ...
            if clientidentifier:
                ...
                display.display_detail("Request failed. Trying %s...", clientidentifier)
                try:
                    manifest = get_manifest(clientidentifier, suppress_errors=True)
                except ManifestNotRetrievedException:
                    pass
...
def clean_up_manifests():
    ...
    exceptions = ["SelfServeManifest"]

    for (dirpath, dummy_dirnames, filenames) in os.walk(manifest_dir, topdown=False):
...
def get_manifest_value_for_key(manifestpath, keyname):
    ...
    try:
        ...
        display.display_error(
            u"Failed to get manifest value for key: %s (%s)", manifestpath, keyname
        )
...
def remove_from_selfserve_section(itemname, section):
    ...
    try:
        ...
        display.display_debug1("Error reading %s: %s", selfservemanifest, err)
        return
    # make sure the section is in the plist
    if section in plist:
        # filter out our item
        plist[section] = [item for item in plist[section] if item != itemname]
        try:
            FoundationPlist.writePlist(plist, selfservemanifest)
        except FoundationPlist.FoundationPlistException as err:
            display.display_debug1("Error writing %s: %s", selfservemanifest, err)
