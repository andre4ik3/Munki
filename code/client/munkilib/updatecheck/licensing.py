...
def update_available_license_seats(installinfo):
    ...
    items_to_check = [
        ...
        if item.get("licensed_seat_info_available") and not item["installed"]
    ]
    ...
    while start_index < len(items_to_check):
        ...
        try:
            ...
            display.display_error("Bad license data from %s: %s", url, license_data)
        else:
    ...
    for item in installinfo["optional_installs"]:
        if item["name"] in items_to_check:
            display.display_debug2("Looking for license info for %s", item["name"])
