...
def config_profile_info(ignore_cache=False):
    ...
    if not ignore_cache and config_profile_info.cache is not None:
        return config_profile_info.cache
    output_plist = os.path.join(tempfile.mkdtemp(dir=osutils.tmpdir()), "profiles")
    ...
    if proc.returncode != 0:
        ...
        try:
            ...
            display.display_error("Could not read configuration profile info: %s" % err)
            config_profile_info.cache = {}
        finally:
            try:
...
def info_for_installed_identifier(identifier, ignore_cache=False):
    ...
    for profile in config_profile_info(ignore_cache=ignore_cache).get(
        "_computerlevel", []
    ):
...
def profile_receipt_data_path():
    ...
    return os.path.join(prefs.pref("ManagedInstallDir"), "ConfigProfileData.plist")
...
def store_profile_receipt_data(identifier, hash_value):
    ...
    if hash_value is not None:
        profile_dict = info_for_installed_identifier(identifier, ignore_cache=True)
    ...
    try:
        ...
        display.display_error("Cannot update hash for %s: %s" % (identifier, err))
...
def read_profile(profile_path):
    ...
    try:
        ...
        display.display_error("Error reading profile %s: %s" % (profile_path, err))
        return {}
...
def read_signed_profile(profile_path):
    ...
    if proc.returncode:
        ...
        display.display_error(
            "Error reading profile %s: %s" % (profile_path, stderr.decode("UTF-8"))
        )
        return {}
    try:
...
def _remove_profile_basic(identifier):
    ...
    stdout = proc.communicate()[0].decode("UTF-8")
    if proc.returncode != 0:
        display.display_error("Profile %s removal failed: %s" % (identifier, stdout))
        return False
    return True
...
def profile_needs_to_be_installed(identifier, hash_value):
    ...
    if not profile_install_supported():
        if not should_emulate_profile_support():
            ...
            return False
    if not in_config_profile_info(identifier) and not localmcx.profile_is_installed(
        identifier
    ):
        display.display_debug2("Profile identifier %s is not installed." % identifier)
        return True
    receipt = get_profile_receipt(identifier)
    if not receipt:
        display.display_debug2("No receipt for profile identifier %s." % identifier)
        return True
    display.display_debug2("Receipt for %s:\n%s" % (identifier, receipt))
    if receipt.get("FileHash") != hash_value:
        display.display_debug2(
            "Receipt FileHash for profile identifier %s does not match." % identifier
        )
        return True
    installed_dict = info_for_installed_identifier(identifier)
    if installed_dict and installed_dict.get("ProfileInstallDate") != receipt.get(
        "ProfileInstallDate"
    ):
        display.display_debug2(
...
def profile_is_installed(identifier):
    ...
    if not profiles_supported():
        ...
        return False
    if in_config_profile_info(identifier):
        display.display_debug2("Profile identifier %s is installed." % identifier)
        return True
