...
def install_profile(profile):
    ...
    if not profile.get("PayloadType") == "Configuration":
        display.display_error(
            "Unsupported profile PayloadType: %s" % profile.get("PayloadType")
        )
        return False
    ...
    if not mcx_data:
        display.display_error(
            ...
            % (profile.get("PayloadIdentifier"), profile.get("PayloadDisplayName"))
        )
        return False
...
def profile_is_installed(identifier):
    ...
    if exitcode:
        display.display_warning("Could not get list of local ComputerGroups: %s" % err)
        return False
...
def add_mcx_to_local_computer_record(recordname):
    ...
    cmd = [
        ...
        ".",
        "mcxset",
        "/Computers/" + recordname,
        "com.github.munki.munki",
        "LocalMCX",
        "always",
        "-int",
        "1",
...
def local_computer_record():
    ...
    if not mac_layer_addr:
        ...
        return None
    cmd = ["/usr/bin/dscl", ".", "search", "/Computers", "ENetAddress", mac_layer_addr]
    ...
    if exitcode == 0 and output:
        ...
        if exitcode:
            display.display_error("Error creating local mcx computer record: %s" % err)
            display.display_error("Cannot create local mcx computer record")
            return None
...
def create_computer_group_with_mcx(groupname, mcx_data):
    ...
    _, err, exitcode = run(cmd)
    if exitcode:
        display.display_error("Error creating computergroup %s: %s" % (groupname, err))
        return False
    mcx_plist = os.path.join(tempfile.mkdtemp(dir=osutils.tmpdir()), "mcx")
...
def delete_computer_group(groupname):
    ...
    _, err, exitcode = run(cmd)
    if exitcode:
        display.display_error("Error deleting computergroup %s: %s" % (groupname, err))
        return False
    return True
...
def domains_handled_by_plugins():
    ...
    for item in os.listdir(xpcservices):
        ...
        try:
            ...
            domains_supported = info.get("ProfileDomainService", {}).get(
                "DomainsSupported"
            )
    ...
    for plugin_dir in plugin_dirs:
        for item in os.listdir(plugin_dir):
            ...
            domains_supported = list(principal_class.new().pdp_pluginDomainsSupported())
            if domains_supported:
                domain_list.extend(domains_supported)
