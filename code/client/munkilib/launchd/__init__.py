...
def job_info(job_label):
    ...
    info = {"state": "unknown", "PID": None, "LastExitStatus": None}
    ...
    if proc.returncode or not output:
        ...
        job_lines = [item for item in lines if item.endswith("\t" + job_label)]
        if len(job_lines) != 1:
...
class Job(object):
    ...
    def __init__(
        self, cmd, environment_vars=None, job_label=None, cleanup_at_exit=True
    ):
    ...
    def start(self):
        ...
        if proc.returncode:
            ...
            if not os.path.exists(self.stdout_path) or not os.path.exists(
                self.stderr_path
            ):
