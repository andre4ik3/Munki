...
try:
    ...
    print (
        'ERROR: could not import module "plistlib." Please install it and ' "try again."
    )
    sys.exit(1)
        ...
        if os.path.isdir(f_path):
            ...
            if not "installer_item_hash" in plist or update_existing:
                ...
                if not os.path.isfile(pkg_path):
                    print >> sys.stderr, (
                        "WARNING: Installer item (%s) not found " "as specified in %s"
                    ) % (pkg_path, f_path)
                    continue
            ...
            if "uninstaller_item_location" in plist:
                if not "uninstaller_item_hash" in plist or update_existing:
                    pkg_path = os.path.join(
                        pkgs_path, plist["uninstaller_item_location"]
                    )
                    # display warning for items that cannot be found.
                    if not os.path.isfile(pkg_path):
                        print >> sys.stderr, (
                            "WARNING: Uninstaller item (%s) not "
                            "found as specified in %s"
                        ) % (pkg_path, f_path)
                        continue
...
def main():
    ...
    p.add_option(
        "-p",
        "--pkgsinfo_dir_name",
        default=MUNKI_PKGSINFO_DIR_NAME,
        help='Munki pkgsinfo dir name; default "pkgsinfo".',
    )
    p.add_option(
        "-f",
        "--pkgs_dir_name",
        default=MUNKI_PKGS_DIR_NAME,
        help='Munki packages dir name; default "pkgs".',
    )
    p.add_option(
        "-u", "--update_existing", action="store_true", help="Update existing hashes."
    )
    ...
    if not os.path.isdir(pkgsinfo_path) or not os.listdir(pkgsinfo_path):
        ...
        AddHashesToPkginfoPlists(pkgsinfo_path, pkgs_path, options.update_existing)
